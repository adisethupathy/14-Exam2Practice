...
class Box(object):
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) > self.volume:
            for k in range(self.volume):
                new_string += str(self.contents[k])
                count += 1

            for i in range(count, len(self.contents)):
                leftover += str(self.contents[i])
    ...
    def double_then_shrink(self, new_volume):
        ...
        jim = self.double()
        pam = self.shrink(new_volume)

        return len(jim) + len(pam)
    ...
    def combined_box(self, other_box):
        ...
        n = Box(self.contents + other_box.contents, self.volume + other_box.volume)
